...
router.delete('/clear-db', async (_, res) => {
    try{
        ...
        console.log("Please check your db has connected");
        res.json({ message: "Please check your db has connected", isFailed: true });
    }
});
...
router.get('/query-cards', async (req, res) => {
    try{
        ...
        if(messages.length!==0){
            res.json({ messages: messages.map((e) => `Found card with ${type}: (${e.name}, ${e.subject}, ${e.score})`), card: messages });
        }else{
            res.json({ message: `${type==="name"?"Name":"Subject"} ${queryString} not found!` });
        }
    }catch{
        console.log("Please check your db has connected");
        res.json({ message: "Please check your db has connected" });         
    }
});
